...
public static void main(String[] args) {
    ...
    try {
        ...
        // transformer la liste de lignes lues (List<String>)
        //en liste de produit (List<Produit>)
        ...
        Stat statHt = new Stat();       
        statHt.setLabel("statHt");
        statHt.setSomme(prixTotalHt);   
        statHt.setMoyenne(prixTotalHt/listeProduits.size());
        statHt.setEcartType(null);      
        Stat statTtc = 
            new Stat("statTtc",prixTotalTtc,
                    prixTotalTtc/listeProduits.size(),null);
        List<Stat> listeStats = new ArrayList<>();
        listeStats.add(statHt);     
         listeStats.add(statTtc); 
        MyCsvUtil.writeValuesAsCsvFile(listeStats,"stats.csv");

        List<Produit> listeProduitsApresSolde = new ArrayList<>();
        for(Produit p : listeProduits) {
            listeProduitsApresSolde.add(new Produit(p.getId(),p.getLabel(),
                          p.getPrixHt()*0.5,p.getTauxTva()));//solde de 50%
        }
        MyCsvUtil.writeValuesAsCsvFile(listeProduitsApresSolde,
                                       "produitsApresSoldes.csv");
      ...
      //générer un fichier produits.json dans le format JSON .
        try {
                ...
                String listeProduitsAsJsonString = 
                    objMapperJackson.writeValueAsString(listeProduits);
                //equivalent java de JSON.stringify()

                /*
                Produit[] tabProduits = 
                objMapperJackson.readValue(listeProduitsAsJsonString, Produit[].class);
                //equivalent java de JSON.parse()
                for(Produit p: tabProduits)
                    System.out.println("p="+p);
                */

                System.out.println("listeProduitsAsJsonString="
                                   +listeProduitsAsJsonString);
